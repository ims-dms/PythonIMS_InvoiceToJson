# System Architecture & Data Flow

## Complete System Flow

```
┌─────────────────────────────────────────────────────────────────────────┐
│                          CLIENT APPLICATION                              │
│                                                                           │
│  User uploads invoice PDF/Image → POST /extract                          │
└────────────────────────────────┬─────────────────────────────────────────┘
                                 │
                                 ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                         FASTAPI SERVER (api.py)                          │
│                                                                           │
│  1. Receive uploaded file                                                │
│  2. Convert PDF to image (if needed)                                     │
└────────────────────────────────┬─────────────────────────────────────────┘
                                 │
                                 ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                      GEMINI AI OCR PROCESSING                            │
│                                                                           │
│  Extract structured data from invoice:                                   │
│  • Invoice metadata (invoice_no, date, dealer_name, etc.)                │
│  • Product list with SKU descriptions                                    │
│  • Quantities, rates, batch numbers, etc.                                │
└────────────────────────────────┬─────────────────────────────────────────┘
                                 │
                                 ▼
                          OCR Result JSON
                    {
                      "products": [
                        {
                          "sku": "LACTOGEN PRO1 BIB 24x400g...",
                          "sku_code": "12579462",
                          "quantity": 5,
                          ...
                        }
                      ]
                    }
                                 │
                                 ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                    DATABASE QUERY (db_connection.py)                     │
│                                                                           │
│  SELECT desca, menucode FROM menuitem                                       │
└────────────────────────────────┬─────────────────────────────────────────┘
                                 │
                                 ▼
                      Check Cache (menu_cache.py)
                                 │
                    ┌────────────┴────────────┐
                    │                         │
              Cache Hit (<1ms)          Cache Miss (1-2s)
                    │                         │
                    │                         ▼
                    │              ┌──────────────────────┐
                    │              │  Query Database      │
                    │              │  Load 700k items     │
                    │              │  Store in cache      │
                    │              └──────────┬───────────┘
                    │                         │
                    └────────────┬────────────┘
                                 │
                                 ▼
                        Menu Items List
                    [
                      ("LACTOGEN PRO 1 BIB 24x400g", "menucode_001"),
                      ("LACTOGEN PRO 2 BIB 24x400g", "menucode_002"),
                      ...
                      (700,000 items)
                    ]
                                 │
                                 ▼
┌─────────────────────────────────────────────────────────────────────────┐
│              FUZZY MATCHING ENGINE (fuzzy_matcher.py)                    │
│                                                                           │
│  For each OCR product SKU:                                               │
│  1. Use RapidFuzz process.extract()                                      │
│  2. Apply token_set_ratio scorer                                         │
│  3. Find top 3 matches above 60% threshold                               │
│  4. Calculate confidence level                                           │
│                                                                           │
│  Algorithm: token_set_ratio                                              │
│  • Handles missing/extra words                                           │
│  • Tolerates typos and spacing                                           │
│  • Ignores word order                                                    │
│                                                                           │
│  Performance: 50-200ms per query (for 700k items)                        │
└────────────────────────────────┬─────────────────────────────────────────┘
                                 │
                                 ▼
                         Enhanced Products
                    [
                      {
                        "sku": "LACTOGEN PRO1...",
                        "quantity": 5,
                        "fuzzy_matches": [
                          {
                            "desca": "LACTOGEN PRO 1...",
                            "menucode": "menucode_001",
                            "score": 94.59,
                            "rank": 1
                          },
                          ...
                        ],
                        "best_match": {...},
                        "match_confidence": "high"
                      }
                    ]
                                 │
                                 ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                          API RESPONSE                                    │
│                                                                           │
│  Return complete JSON with:                                              │
│  • Original OCR data                                                     │
│  • Enhanced products with fuzzy matches                                  │
│  • Confidence scores                                                     │
│  • Match suggestions                                                     │
└────────────────────────────────┬─────────────────────────────────────────┘
                                 │
                                 ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                      CLIENT DECISION LOGIC                               │
│                                                                           │
│  IF match_confidence == "high" (score ≥ 85):                             │
│    → Auto-accept best_match.menucode                                        │
│                                                                           │
│  ELIF match_confidence in ["medium", "low"] (score 60-84):               │
│    → Show fuzzy_matches to user for selection                            │
│                                                                           │
│  ELSE match_confidence == "none" (score < 60):                           │
│    → Require manual entry                                                │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## Component Interaction Diagram

```
┌──────────────┐
│   api.py     │  ← Main FastAPI application
└──────┬───────┘
       │
       ├─→ ┌──────────────────┐
       │   │ db_connection.py │  ← Database connectivity
       │   └──────────────────┘
       │
       ├─→ ┌──────────────────┐
       │   │ menu_cache.py    │  ← Caching layer (1hr TTL)
       │   └──────────────────┘
       │
       └─→ ┌──────────────────┐
           │ fuzzy_matcher.py │  ← RapidFuzz matching engine
           └──────────────────┘
```

---

## Cache Lifecycle

```
Request 1 (t=0s):
┌─────────────────┐
│ Cache Empty     │ → Query DB (1-2s) → Store in Cache → Return data
└─────────────────┘

Request 2 (t=30s):
┌─────────────────┐
│ Cache Valid     │ → Return from Cache (<1ms) → Fast response
└─────────────────┘

Request 3 (t=3600s):
┌─────────────────┐
│ Cache Expired   │ → Query DB (1-2s) → Refresh Cache → Return data
└─────────────────┘

Manual Invalidation:
┌─────────────────┐
│ POST            │
│ /cache/         │ → Clear Cache → Next request loads fresh data
│ invalidate      │
└─────────────────┘
```

---

## Fuzzy Matching Process Detail

```
Input: "LACTOGEN PRO1 BIB 24x400g INNWPB176 NP"
                        │
                        ▼
┌──────────────────────────────────────────────────────┐
│ RapidFuzz process.extract()                          │
│                                                       │
│ Compares against 700,000 menu items using:          │
│ • Scorer: token_set_ratio                           │
│ • Limit: Top 3 matches                              │
│ • Cutoff: Minimum 60% similarity                    │
└──────────────────────────────────────────────────────┘
                        │
                        ▼
        Match Results (sorted by score)
┌──────────────────────────────────────────────────────┐
│ Rank 1: "LACTOGEN PRO 1 BIB 24x400g INNWPB176"      │
│         menucode_001, Score: 94.59                     │
│         ↑ Best Match                                 │
├──────────────────────────────────────────────────────┤
│ Rank 2: "LACTOGEN PRO 2 BIB 24x400g INLEB086"      │
│         menucode_002, Score: 72.35                     │
├──────────────────────────────────────────────────────┤
│ Rank 3: "LACTOGEN PRO 3 FOLLOW UP 400g"            │
│         menucode_003, Score: 65.12                     │
└──────────────────────────────────────────────────────┘
                        │
                        ▼
              Confidence Classification
┌──────────────────────────────────────────────────────┐
│ Best match score: 94.59                             │
│ 94.59 ≥ 85 → Confidence: "high"                     │
│                                                       │
│ Decision: Auto-accept menucode_001                      │
└──────────────────────────────────────────────────────┘
```

---

## Performance Metrics Flow

```
┌─────────────────────────────────────────────────────────────┐
│                     REQUEST TIMELINE                         │
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  0ms ─────────┐                                              │
│               │ Receive Request                              │
│  1ms ─────────┘                                              │
│               │                                              │
│               │ Check Cache                                  │
│               │                                              │
│  2ms ─────────┴─ Cache Hit? ─┬─ YES → <1ms                 │
│                                │                             │
│                                └─ NO  → Query DB (1000-2000ms)│
│                                                               │
│  3ms ─────────┐                                              │
│               │ Start Fuzzy Matching                         │
│               │ (per product)                                │
│               │                                              │
│  53ms ────────┘ Product 1 matched (50ms)                     │
│  103ms ─────── Product 2 matched (50ms)                      │
│  153ms ─────── Product 3 matched (50ms)                      │
│               │                                              │
│  155ms ────────┐                                             │
│               │ Build Response                               │
│  160ms ────────┘                                             │
│               │                                              │
│  160ms ────────┐                                             │
│               │ Return to Client                             │
│  165ms ────────┘                                             │
│                                                               │
│  TOTAL (Cache Hit):  ~160ms                                  │
│  TOTAL (Cache Miss): ~2000ms (first request only)            │
│                                                               │
└─────────────────────────────────────────────────────────────┘
```

---

## Data Structures

### Menu Items (Database)
```python
[
    ("LACTOGEN PRO 1 BIB 24x400g INNWPB176", "menucode_001"),
    ("LACTOGEN PRO 2 BIB 24x400g INLEB086", "menucode_002"),
    ...
    # 700,000 tuples (desca, menucode)
]
```

### OCR Products (Input)
```python
[
    {
        "sku": "LACTOGEN PRO1 BIB 24x400g INNWPB176 NP",
        "sku_code": "12579462",
        "quantity": 5,
        "rate": 872.82,
        "mrp": 15898.56
    },
    ...
]
```

### Enhanced Products (Output)
```python
[
    {
        "sku": "LACTOGEN PRO1 BIB 24x400g INNWPB176 NP",
        "sku_code": "12579462",
        "quantity": 5,
        "rate": 872.82,
        "mrp": 15898.56,
        
        # NEW: Fuzzy matching results
        "fuzzy_matches": [
            {
                "desca": "LACTOGEN PRO 1 BIB 24x400g INNWPB176",
                "menucode": "menucode_001",
                "score": 94.59,
                "rank": 1
            },
            ...
        ],
        "best_match": {...},
        "match_confidence": "high"
    },
    ...
]
```

---

## API Endpoints Flow

```
┌─────────────────────────────────────────────────────────────┐
│                     API ENDPOINTS                            │
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  GET /                                                        │
│  └─→ Health check                                            │
│                                                               │
│  POST /extract                                                │
│  ├─→ Receive invoice file                                    │
│  ├─→ OCR with Gemini                                         │
│  ├─→ Fetch menu items (cached)                               │
│  ├─→ Apply fuzzy matching                                    │
│  └─→ Return enhanced products                                │
│                                                               │
│  GET /cache/status                                            │
│  └─→ {                                                        │
│        "cache": {                                             │
│          "status": "valid",                                   │
│          "item_count": 700000,                                │
│          "age_seconds": 1234.56,                              │
│          "expires_in": 2365.44                                │
│        }                                                      │
│      }                                                        │
│                                                               │
│  POST /cache/invalidate                                       │
│  └─→ Force cache refresh                                     │
│                                                               │
└─────────────────────────────────────────────────────────────┘
```

---

## Error Handling Flow

```
┌─────────────────────────────────────────────────────────────┐
│                    ERROR SCENARIOS                           │
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  Database Connection Failed                                  │
│  └─→ Raise HTTPException(500)                                │
│      └─→ Log to tblOCRTokenDetails                           │
│                                                               │
│  OCR Extraction Failed                                       │
│  └─→ Raise HTTPException(422)                                │
│      └─→ Return error details                                │
│                                                               │
│  Cache Invalid                                               │
│  └─→ Attempt DB query                                        │
│      └─→ If fails, raise error                               │
│                                                               │
│  No Fuzzy Matches Found                                      │
│  └─→ Return empty fuzzy_matches: []                          │
│      └─→ match_confidence: "none"                            │
│      └─→ Client handles manual entry                         │
│                                                               │
│  Low Confidence Match (60-69%)                               │
│  └─→ Return matches with "low" confidence                    │
│      └─→ Client shows for user review                        │
│                                                               │
└─────────────────────────────────────────────────────────────┘
```

This completes the visual documentation of your high-performance fuzzy matching system! 🎉
